...
class PathgraphomicDatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "path" or self.mode == 'path_effnet':
            ...
            if 'TCGA_GBMLGG' in self.data_name:
                single_X_grph = torch.load(self.newgraphroot+ self.X_grph[index][30:])
            elif 'TCGA_KIRC' in self.data_name:
                single_X_grph = torch.load(self.newgraphroot+ self.X_grph[index][35:])
...
class PathgraphomicFastDatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "path" or self.mode == 'pathpath' or self.mode == 'path_Unet':
            ...
            single_X_grph = torch.load(self.newgraphroot+ self.X_grph[index][42:end] + '.pt')
            return (0, single_X_grph, 0, single_e, single_t, single_g)
