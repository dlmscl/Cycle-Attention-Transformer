...
def define_net(opt, k):
    ...
    elif opt.mode == 'path_effnet':
        efficientnet = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_efficientnet_b0', pretrained=True)
        utils = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_convnets_processing_utils')
        efficientnet.eval()
        net = PathNet(efficientnet, act=act, num_classes=opt.label_dim, path_dim=opt.path_dim, mode=opt.mode)
    elif opt.mode == "graph":
        net = GraphNet(grph_dim=opt.grph_dim, dropout_rate=opt.dropout_rate, GNN=opt.GNN, use_edges=opt.use_edges, pooling_ratio=opt.pooling_ratio, act=act, label_dim=opt.label_dim, init_max=init_max,graph_model = opt.graph_model)
    elif opt.mode == "omic":
...
def define_optimizer(opt, model): 
    ...
    if opt.optimizer_type == 'adabound':
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2), weight_decay=opt.weight_decay)
    elif opt.optimizer_type == 'adan':
        optimizer = Adan(model.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
    else:
...
def define_reg(opt, model):
    ...
    if opt.reg_type == 'none':
        ...
        loss_reg = regularize_MM_path(model=model)
    ...
    elif opt.reg_type == 'decoder':
        loss_reg = regularize_decoder_weights(model=model)
    elif opt.reg_type == 'pathomic':
        loss_reg = regularize_MM_pathomic(model=model)
    else:
...
class GraphNet(torch.nn.Module):
    def __init__(self, features=1036, nhid=128, grph_dim=32, nonlinearity=torch.tanh, 
        dropout_rate=0.25, GNN='GCN', use_edges=0, pooling_ratio=0.20, act=None, label_dim=1, init_max=True, graph_model='SAGE'):
        ...
        if graph_model == 'SAGE':
            ...
            self.pool1 = SAGPooling(nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.conv.SAGEConv) 
            self.conv2 = SAGEConv(nhid, nhid)
            self.pool2 = SAGPooling(nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.conv.SAGEConv) 
            self.conv3 = SAGEConv(nhid, nhid)
            self.pool3 = SAGPooling(nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.conv.SAGEConv)
        elif graph_model == 'GCN':
            self.conv1 = SAGEConv(features, nhid)   
            self.pool1 = SAGPooling(nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.conv.GCNConv) 
            self.conv2 = SAGEConv(nhid, nhid)
            self.pool2 = SAGPooling(nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.conv.GCNConv) 
            self.conv3 = SAGEConv(nhid, nhid)
            self.pool3 = SAGPooling(nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.conv.GCNConv)
    ...
    def forward(self, **kwargs):
        ...
        x, edge_index, edge_attr, batch, _ , _d = self.pool1(x, edge_index, edge_attr, batch)
        x1 = torch.cat([gmp(x, batch), gap(x, batch)], dim=1)

        x = F.relu(self.conv2(x, edge_index))
        x, edge_index, edge_attr, batch, _ , _d = self.pool2(x, edge_index, edge_attr, batch)
        x2 = torch.cat([gmp(x, batch), gap(x, batch)], dim=1)

        x = F.relu(self.conv3(x, edge_index))
        x, edge_index, edge_attr, batch, _ , _d = self.pool3(x, edge_index, edge_attr, batch)
...
class PathNet(nn.Module):
    ...
    def __init__(self, features, path_dim=32, act=None, num_classes=1, mode='path'):
...
class GraphomicNet(nn.Module):
    def __init__(self, opt, act, k):
        ...
        self.grph_net = GraphNet(grph_dim=opt.grph_dim, dropout_rate=opt.dropout_rate, use_edges=1, pooling_ratio=0.20, label_dim=opt.label_dim, init_max=False,graph_model = opt.graph_model)
        ...
        self.grph_net = nn.DataParallel(self.grph_net)
        self.omic_net = nn.DataParallel(self.omic_net)
        ...
        elif opt.myfusion=="CrossAttention":
            self.fusion=Bifusion_CrossAttention(opt, heads=opt.Numhead,num=opt.Tfnum,dropout=opt.dropout_rate, forward_expansion=4, max_len=5000)
...
class PathomicNet(nn.Module):
    def __init__(self, opt, act, k):
        ...
        self.use_vgg_features = opt.use_vgg_features
        ...
        if opt.use_vgg_features == 0:
                efficientnet = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_efficientnet_b0', pretrained=True)
                efficientnet.eval()
                self.path_net = PathNet(efficientnet, act=act, num_classes=opt.label_dim, path_dim=opt.path_dim, mode='path_effnet')
                self.path_net = nn.DataParallel(self.path_net)
                
                best_path_ckpt = torch.load(os.path.join(opt.checkpoints_dir, opt.exp_name, 'path_effnet', 'path_effnet'+pt_fname), map_location=torch.device('cpu'))
                self.path_net.load_state_dict(best_omic_ckpt['model_state_dict'])
                print("Loading Models:\n", os.path.join(opt.checkpoints_dir, opt.exp_name, 'path_effnet', 'path_effnet'+pt_fname))
...
class PathgraphomicNet(nn.Module):
    def __init__(self, opt, act, k):
        ...
        elif opt.myfusion=="Trifusion_CrossAttention":
            self.fusion=Trifusion_CrossAttention(opt,embed_size=opt.omic_dim, heads=opt.Numhead, part=4,num=opt.Tfnum,dropout=opt.dropout_rate, forward_expansion=4, max_len=5000)
        elif opt.myfusion=="MULT_fusion":          self.fusion=MULTmodel_old(opt,embed_size=opt.omic_dim,heads=opt.Numhead,part=4,Conly=opt.Conly,Gonly=opt.Gonly,Sonly=opt.Sonly,layers=opt.Tfnum,dropout=opt.dropout_rate, forward_expansion=4, max_len=5000)
        elif opt.myfusion=="Cy_Atten":
            self.fusion=MULTmodel(opt,heads=opt.Numhead,dropout=opt.dropout_rate, forward_expansion=4, max_len=5000)    
