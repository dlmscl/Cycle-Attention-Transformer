...
In this work, we present a Cycle-Attention Transformer (Cy-Atten) framework that learns a more interpretable and robust joint representation for multimodal cancer survival analysis and is proven to effectively reduce the parameter redundancy via a cycling mechanism with sparse attention. The Cy-Atten yields a theoretical cost of $O(N)$ for adding a modality. As a comparison, an existing multimodal fusion method based on Kronecker Product, Pathomic Fusion (PaF) [^1], yields a theoretical cost of $O((m+1)^N)$ (m is a constant) for adding a modality. Empirically in the experiments on two canser datasets from the TCGA database, the parameters in our fusion part are only of the order of $\mathbf{10^{3}}$ considering three modalities while the parameters in the fusion part of PaF [^1] are of the order of $\mathbf{10^6}$. In addition to being lightweight, the Cy-Atten outperforms several state-of-the-art multimodal fusion methods in survival outcome prediction and grade classification of two cancers from the TCGA database.
