...
from utils import makeAUROCPlot, GradMetrics
...
if 'TCGA_GBMLGG' in opt.dataroot:
...
elif 'TCGA_KIRC' in opt.dataroot:
    if not opt.use_vgg_features:
        data_cv_path = '%s/splits/KIRC_st_0.pkl' % opt.dataroot
    else:
        data_cv_path = '%s/splits/KIRC_st_1_ag.pkl' % opt.dataroot


print("Loading %s" % data_cv_path, file=file)
data_cv = pickle.load(open(data_cv_path, 'rb'))

if 'TCGA_KIRC' in opt.dataroot and opt.task == 'surv' and (opt.mode == 'graphomic' or opt.mode == 'graph' or opt.mode == 'pathgraphomic'):
    for i in range(1,16):
