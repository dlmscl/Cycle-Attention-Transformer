...
# Regularization
################
def regularize_decoder_weights(model, reg_type=None):
    l1_reg = None
    
    for W in model.module.fusion.parameters():
        if l1_reg is None:
    ...
    for W in model.module.act.parameters():
        l1_reg = l1_reg + torch.abs(W).sum()
...
def regularize_MM_path(model, reg_type=None):
    l1_reg = None
    # print(model)
    if model.module.__hasattr__('path_net'):
        for W in model.module.omic_net.parameters():
            if l1_reg is None:
                l1_reg = torch.abs(W).sum()
            else:
...
def regularize_MM_pathomic(model, reg_type=None):
    l1_reg = None
    # print(model)
    if model.module.__hasattr__('path_net'):
        for W in model.module.omic_net.parameters():
...
def init_max_weights(module,myfusion=1):
...
#     use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 0) if ((('path' in model) or ('graph' in model)) and ('cox' not in model)) else ('_', 'all_st', 0)
#     use_rnaseq = '_rnaseq' if ('rnaseq' in ckpt_name and 'path' != model and 'pathpath' not in model and 'graph' != model and 'graphgraph' not in model) else ''
    ...
    for k in range(1,16):
        ...
        pred = torch.load(ckpt_name + 'pred_test_' + model + "/_k" + str(k) + '.pt' )
        pred = pred['pred']
...
#             data_cv = pickle.load(open("./data/TCGA_GBMLGG/splits/gbmlgg15cv_all_st_1_0_0_rnaseq.pkl", 'rb'))
            data_cv = pickle.load(open("./data/TCGA_GBMLGG/splits/gbmlgg15cv_all_st_patches_512_1_0_1_rnaseq.pkl", 'rb'))
...
def calcGradMetrics(ckpt_name='./checkpoints/grad_15/', model='pathgraphomic_fusion', split='test', avg='micro'):
    ...
    for k in range(1,16):
        pred = torch.load(ckpt_name + "_" + model + '/pred_test' + str(k) + '.pt')
...
#Added a new function based on calcGradMetrics
def GradMetrics(pred, model='pathgraphomic_fusion', split='test', avg='micro'):
    auc_all = []
    ap_all = []
    f1_all = []
    f1_gradeIV_all = []
...
def makeHazardSwarmPlot(ckpt_name='./checkpoints/surv_15_rnaseq/', model='path', split='test', zscore=True, agg_type='Hazard_mean'):
    ...
    fig.savefig('lsc_log'+'/hazard_picture/%s_HSP.png' % (model))

def makeHazardDistPlot(ckpt_name='./checkpoints/surv_15_rnaseq/', model='path', split='test', zscore=True, agg_type='Hazard_mean'):
    
    data = poolSurvTestPD(ckpt_name=ckpt_name, model=model, split=split, zscore=zscore, agg_type=agg_type)
    print(len(data['Hazard']))
    print(len(data[data['Survival months'] > 1825]['Hazard']))
    print(len(data[data['Survival months'] < 1825]['Hazard']))
    
    fig, ax = plt.subplots(dpi=600)
    sns.distplot(a=data[data['Survival months'] < 1825]['Hazard'], label="Patient Survival < 5 years", kde=True)
    sns.distplot(a=data[data['Survival months'] > 1825]['Hazard'], label="Patient Survival > 5 years", kde=True)
    ax.legend(prop={'size': 8})
    
    fig.savefig('lsc_log'+'/hazard_picture/%s_HDP.png' % (model))
...
def makeAUROCPlot(ckpt_name='./checkpoints/grad_15/', model_list=['CrossAttention(PathOmic)', 'CrossAttention(graphomic)', 'Cycle Transformer', 'TriBERT', 'MULT'], split='test', avg='micro', use_zoom=False):
    mpl.rcParams['font.family'] = "arial"
    colors = {'CrossAttention(PathOmic)':'dodgerblue', 'CrossAttention(graphomic)':'orange', 'TriBERT':'green', 'Cycle Transformer':'crimson', 'MULT':'darkviolet'}
    names = {'CrossAttention(PathOmic)':'Cro-Atten(PO)', 'CrossAttention(graphomic)':'Cro-Atten(GO)', 'TriBERT':'Tri-Co-Atten(PGO)', 'Cycle Transformer':'Cy-Atten(PGO)', 'MULT':'MulT(PGO)'}
    
    zoom_params = {0:([0.1, 0.3], [0.62, 0.82]), 
                   1:([0.25, 0.45], [0.75, 0.95]),
                   2:([0.0, 0.2], [0.8, 1.0]),
                   'micro':([0.02, 0.2], [0.6, 0.8])}
    ...
    for i in classes:
        ...
        for m, model in enumerate(model_list):
            ...
            for k in range(1,16):
                checkpoint = torch.load(ckpt_name + 'pred_test_' + model + "/_k" + str(k) + '.pt' )
                ...
                if i != avg:
                    ...
                    rocaucs.append(roc_auc_score(grade_oh, grade_pred, average=avg))
                    tprs.append(interp(mean_fpr, fpr, tpr))
                    tprs[-1][0] = 0.0
            ...
            if use_zoom:
                alpha, lw = (0.8, 6) if model =='pathgraphomic' else (0.5, 6)
